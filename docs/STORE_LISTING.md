...
Firefox uses authorization code + PKCE with a Desktop app OAuth client ID. The Firefox build sends the authorization code and one-time verifier directly to Google's token endpoint, retains only a short-lived access token in memory, and does not store a client secret or refresh token. Confirm the production client configuration and live Google flow before submitting this text to AMO. The data collection declaration is tracked in #28.

Firefox additionally requests access to `https://oauth2.googleapis.com/token` solely for the one-time PKCE code exchange. It sends no Sheet content or popup search text to that endpoint.
